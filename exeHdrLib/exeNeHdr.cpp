...
NeExeHdr::NeExeHdr( unsigned char *lpByte,
 ...
 */
bool NeExeHdr::HasWinNeHeader( BYTE *lpByte,    ///< pointer to memory mapped file data
                  unsigned long long   ulFileSize ) ///< size of allocated buffer );    // does the file data represent a DOS PE EXE file)
{
  ...
  return  ( (*(USHORT *)lpByte == DOS_IMAGE_MAGIC) &&
      ...
      (LOWORD (*(DWORD *)NTSIGNATURE (lpByte)) == IMAGE_OS2_SIGNATURE) ); 
}
 ...
 */
bool NeExeHdr::DisplayNeHeader( wxTreeCtrl *pTree,
                                wxTreeItemId &parentId )
{
  ...
  newNeExeHdr_t *pNeHdr = 
    (newNeExeHdr_t *)(mp_lExeFileData + pDosHdr->ulNewHdrOffs );
  // Linker
